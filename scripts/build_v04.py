...
t = t.replace(old_people_intro, new_people_intro, 1)

# 用函式邊界取代，避免縮排／換行差異造成失敗
new_render = r'''function renderPeople(){
...
if n != 1:
    raise RuntimeError(f"renderPeople 取代失敗：{n}")

new_norm = r'''function normalizeDateInput(s){
...
t, n = re.subn(
    r"function normalizeDateInput\(s\)\{.*?\n\}",
    lambda m: new_norm,
    t,
    count=1,
    flags=re.S,
...
if n != 1:
    raise RuntimeError(f"normalizeDateInput 取代失敗：{n}")

# 發布後不得依賴 RawGitHack/Jotform；來源抓取只發生在本次建置
if "raw.githack" in t.lower() or "jotform" in t.lower():
    raise RuntimeError("輸出仍含第三方入口字樣")
if "baseball-GM" in t or "raw.githubusercontent.com" in t:
    raise RuntimeError("輸出 index.html 仍有舊 repository 執行期依賴")

with open("index.html", "w", encoding="utf-8", newline="\n") as f:
    f.write(t)

print("built standalone index.html", len(t), "chars")
